...
static volatile uint8_t g_usbx_device_ready = 0u;
...
{
  if (g_usbx_device_ready == 0u) {
    return 0u;
  }

  if (UsbDevice_CdcReady() != 0u) {
    return 1u;
  }
  ...
  if (dev->ux_slave_device_configuration_selected != 0UL) {
    return 1u;
  }

  if (dev->ux_slave_device_state == UX_DEVICE_CONFIGURED) {
    return 1u;
  }

  uint32_t const daddr = hpcd_USB_DRD_FS.Instance->DADDR;
  if (((daddr & USB_DADDR_EF) != 0U) && ((daddr & USB_DADDR_ADD) != 0U)) {
    return 1u;
  }
